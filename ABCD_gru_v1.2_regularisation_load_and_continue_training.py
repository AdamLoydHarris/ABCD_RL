...
if __name__ == "__main__":
    ...
    with open('gru_outputs/training_tasks.txt', 'r') as file:
        training_reward_orders = [list(map(int, line.strip()[1:-1].split(', '))) for line in file.readlines()]
    
    num_training_orders = len(training_reward_orders)
    ...
    model = ActorCritic(input_size=15, hidden_size=300, num_actions=4)
    model.load_state_dict(torch.load("gru_outputs/gru_actor_critic_ABCD.pth"))
    model.train()
    ...
    torch.save(model.state_dict(), "gru_outputs/gru_actor_critic_ABCD_updated.pth")
    ...
    with open('gru_outputs/test_tasks.txt', 'r') as file:
        unseen_orders = [list(map(int, line.strip()[1:-1].split(', '))) for line in file.readlines()]

    num_test_orders = len(unseen_orders)
