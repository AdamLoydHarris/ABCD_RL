...
class ActorCritic(nn.Module):
    ...
    def init_hidden(self, batch_size=1):
        # LSTM hidden state includes both hidden state and cell state
...
def evaluate_agent(env, model):
    ...
    while not done:
        logits, value, hidden = model(obs_tensor, hidden)
        activation = hidden[0].detach().cpu().numpy()[0, 0, :].copy()
...
def plot_test_session(activations, reward_events, unit_order=None, session_idx=0):
    ...
    plt.ylabel("LSTM Unit (Rank-ordered)")
    plt.title(f"Test Session {session_idx+1} - LSTM Activations")
...
if __name__ == "__main__":
    ...
    with open("lstm_outputs/training_tasks.txt", "w") as f:
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig("lstm_outputs/training_rewards_plot.svg")
    plt.show()
    ...
    with open("lstm_outputs/test_tasks.txt", "w") as f:
    ...
    with open("lstm_outputs/test_sessions_data.pkl", "wb") as f:
