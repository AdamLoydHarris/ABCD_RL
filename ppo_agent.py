...
class PPOAgent:
    ...
    def select_action(self, state, hidden_state):
        state = torch.FloatTensor(state).unsqueeze(0)  # Expand to [1, sequence_length, state_dim]
    ...
    def update(self, memory):
        ...
        hidden_state = self.model.init_hidden_state(batch_size=states.size(0))
        ...
        for _ in range(4):  # PPO epochs
            ...
            values = values.squeeze(-1)  # Remove the last dimension if it is 1
    ...
    def train(self, env, num_episodes=1000, rollout_length=100):
        for episode in range(num_episodes):
            state = env.reset()
            hidden_state = self.model.init_hidden_state(batch_size=1)
    ...
    def get_combined_state(self, state, previous_action, previous_reward):
        combined_state = [float(state[0]), float(state[1]), float(previous_action), float(previous_reward)]
...
#     def select_action(self, state, hidden_state):
#         state = torch.FloatTensor(state).unsqueeze(0)  # Ensure the state is a FloatTensor
#         policy_logits, value, hidden_state = self.model(state, hidden_state)
#         probs = Categorical(logits=policy_logits)
#         action = probs.sample()
#         return action.item(), value, probs.log_prob(action), probs.entropy(), hidden_state
...
#         combined_state = [state[0], state[1], float(previous_action), float(previous_reward)]
#         return combined_state
